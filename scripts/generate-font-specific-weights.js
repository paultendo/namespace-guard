 ...
 *
 * Converts a confusable-vision per-font weights file (font-specific-weights-v3.json, from a release) into a
 * TypeScript file exporting a FONT_SPECIFIC_WEIGHTS constant for use by namespace-guard.
 *
 ...
 *   - danger = how close the pair is in that font (1 identical, 0 at the alike threshold)
 *   - stableDanger = same value (single font, no aggregation)
 *   - cost = 1 - danger
 *
 * The file lists only pairs that pass the release's thresholds, in the fonts where each is alike.
 *
 * Usage:
 *   node scripts/generate-font-specific-weights.js [path-to-json]
 *
 * Default path: ../confusable-vision/data/output/font-specific-weights-v3.json
 */
...
const DEFAULT_JSON_PATH = path.resolve(
  __dirname,
  "../../confusable-vision/data/output/font-specific-weights-v3.json"
);
...
if (!fs.existsSync(jsonPath)) {
  ...
  console.error("Run confusable-vision scripts/generate-font-weights-release.ts first.");
  process.exit(1);
}
...
// Source: confusable-vision ${path.basename(jsonPath)} (${data.meta.fontCount} fonts, ${data.meta.pairCount} pairs, release ${data.meta.release})
...
/**
 * Per-font confusable weight maps for ${data.meta.fontCount} fonts (macOS system fonts and Roboto).
 *
...
for (const fontName of fontNames) {
  ...
  const highRiskEdges = fontData.edges;
  ...
  for (const edge of highRiskEdges) {
    ...
    const danger = round(edge.similarity);
