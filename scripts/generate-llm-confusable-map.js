...
for (const edge of weightsData.edges) {
  ...
  // A pair can be found alike both within one font and across fonts; keep the stronger finding
  const prior = scoredEdgeByPair.get(key);
  if (prior && (prior.danger ?? 0) >= (edge.danger ?? 0)) continue;
...
out += `// Sources: CONFUSABLE_MAP_FULL (TR39 baseline) + confusable-vision ${path.basename(weightsPath)}${weightsData.meta?.release ? ` (release ${weightsData.meta.release})` : ""}\n`;
